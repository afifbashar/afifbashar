...
require_once 'db.php';
$id = $_GET['id'];
$stmt = $pdo->prepare("SELECT * FROM guidelines WHERE id = ?");
$stmt->execute([$id]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $pdo->prepare("UPDATE guidelines SET title = ?, content = ?, category_id = ? WHERE id = ?");
...
<body class="bg-gray-50 min-h-screen">
    <div class="container mx-auto p-4 md:p-6">
        <h1 class="text-2xl md:text-3xl font-bold text-gray-800 mb-6">Edit Guideline</h1>
        <form method="POST" enctype="multipart/form-data" class="bg-white p-6 rounded-lg shadow-md">
            <div class="mb-4">
                <label class="block text-gray-700 font-medium mb-1">Title</label>
                <input type="text" name="title" value="<?php echo htmlspecialchars($guideline['title']); ?>" class="w-full border p-2 rounded-lg focus:ring-2 focus:ring-blue-300 outline-none" required>
            </div>
            <div class="mb-4">
                ...
                <select name="category_id" class="w-full border p-2 rounded-lg focus:ring-2 focus:ring-blue-300 outline-none">
                    ...
                    while ($cat = $stmt->fetch()) {
                        $selected = $cat['id'] == $guideline['category_id'] ? 'selected' : '';
            ...
            <div class="mb-4">
                ...
                <textarea name="content" id="editor" class="w-full border rounded-lg"><?php echo htmlspecialchars($guideline['content']); ?></textarea>
            </div>
